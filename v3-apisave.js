javascript:(function() {
    ...
    class SecureSettings {
        constructor() { 
            ...
            this.storageKey = SECURITY_NAMESPACE + '_conf';
            // Enhanced memory storage for API keys
        ...
        save() { 
            try { 
                ...
                const dataToStore = { ...this.current };
                localStorage.setItem(this.storageKey, this.obfuscate(dataToStore)); 
                
                // Also store in memory as backup
                this.memoryStore.set(this.storageKey, { ...this.current });
                
                // Store API keys separately in memory with timestamp
                this.memoryStore.set('apiKeys', {
                    gemini: this.current.apiKey,
                    cohere: this.current.cohereKey,
                    huggingface: this.current.huggingfaceKey,
                    timestamp: Date.now()
                });
                
                console.log('Settings saved successfully');
            } catch (e) { 
                console.warn('LocalStorage failed, using memory fallback:', e);
                // If localStorage fails, ensure memory storage is updated
                this.memoryStore.set(this.storageKey, { ...this.current });
            } 
        }
        ...
        load() { 
            try { 
                ...
                if (stored) {
                    const decoded = this.deobfuscate(stored);
                    if (decoded) {
                        this.current = { ...this.defaults, ...decoded };
                        // Update memory store
                        this.memoryStore.set(this.storageKey, { ...this.current });
                ...
                const memoryData = this.memoryStore.get(this.storageKey);
                if (memoryData) {
                    this.current = { ...this.defaults, ...memoryData };
                    console.log('Settings loaded from memory');
                    return;
                }
                
                // Load API keys from memory if available
                const apiKeys = this.memoryStore.get('apiKeys');
                if (apiKeys && (Date.now() - apiKeys.timestamp < 86400000)) { // 24 hours
                    this.current.apiKey = apiKeys.gemini || '';
                    this.current.cohereKey = apiKeys.cohere || '';
                    this.current.huggingfaceKey = apiKeys.huggingface || '';
                    console.log('API keys loaded from memory');
                }
                
            } catch (e) { 
                console.warn('Settings load failed:', e);
                // Try to recover API keys from memory
                const apiKeys = this.memoryStore.get('apiKeys');
                if (apiKeys) {
                    this.current.apiKey = apiKeys.gemini || '';
                    this.current.cohereKey = apiKeys.cohere || '';
        ...
        get(key) { 
            // First check memory store for latest values
            const memoryData = this.memoryStore.get(this.storageKey);
            if (memoryData && memoryData[key] !== undefined) {
        ...
        set(key, value) { 
            ...
            this.memoryStore.set(this.storageKey, memoryData);
            
            // Auto-save with debouncing for API keys
            if (['apiKey', 'cohereKey', 'huggingfaceKey'].includes(key)) {
        ...
        // Enhanced API key retrieval with fallbacks
        getApiKey(provider) {
            const keyMap = {
                'gemini': 'apiKey',
                'cohere': 'cohereKey', 
            ...
            if (!key || key.trim() === '') {
                ...
                if (apiKeys && apiKeys[provider]) {
                    ...
                    // Update current settings with recovered key
    ...
    class SuiteApplication {
        constructor() {
            this.settings = new SecureSettings(); 
            this.settings.load();
            this.chatHistory = new SecureChatHistory(); 
        ...
        buildUI() {
            ...
            <div class="gas-tab-content gas-settings-content">
                 ...
                 <div id="gemini-settings" class="gas-settings-section"><label>Gemini API Key: <input type="password" id="gemini-key" placeholder="AIza..."><small class="api-key-status" id="gemini-status"></small></label></div>
                 <div id="cohere-settings" class="gas-settings-section"><label>Cohere API Key: <input type="password" id="cohere-key" placeholder="Your Cohere key..."><small class="api-key-status" id="cohere-status"></small></label></div>
                 <div id="huggingface-settings" class="gas-settings-section"><label>Hugging Face API Key: <input type="password" id="huggingface-key" placeholder="hf_..."><small class="api-key-status" id="huggingface-status"></small></label></div>
                 ...
                 <div class="storage-info"><small>API keys are automatically saved and persist across sessions</small></div>
            </div>
        ...
        createStyles(){const s=document.createElement('style');s.textContent=`
            ...
            .api-key-status{display:block;font-size:0.8em;margin-top:4px;opacity:0.8;}
            .api-key-status.valid{color:#4CAF50;}.api-key-status.invalid{color:#f44336;}
            .storage-info{margin-top:10px;text-align:center;opacity:0.7;}
        ...
        addEventListeners() {
            ...
            ['gemini-key', 'cohere-key', 'huggingface-key'].forEach(id => {
                const input = this.shadow.querySelector(`#${id}`);
                this.memory.add(input, 'input', (e) => this.validateApiKeyInput(e));
                this.memory.add(input, 'blur', (e) => this.saveApiKey(e));
            });
        }
        
        validateApiKeyInput(e) {
            const input = e.target;
            const provider = input.id.replace('-key', '').replace('gemini', 'gemini').replace('cohere', 'cohere').replace('huggingface', 'huggingface');
            const statusElement = this.shadow.querySelector(`#${provider}-status`);
            
            if (this.settings.validateApiKey(provider, input.value)) {
        ...
        saveApiKey(e) {
            ...
            const keyMap = {
                ...
                'huggingface-key': 'huggingfaceKey'
            };
            
            const settingKey = keyMap[input.id];
            if (settingKey && input.value.trim() !== '') {
                this.settings.set(settingKey, input.value.trim());
                console.log(`${settingKey} saved automatically`);
            }
        }
        ...
        handleTabClick(e){
            this.shadow.querySelectorAll('.gas-tab').forEach(t=>t.classList.remove('active'));
            e.target.classList.add('active');
            const isSettings=e.target.dataset.tab==='settings';
            this.shadow.querySelector('.gas-chat-content').style.display=isSettings?'none':'flex';
            this.shadow.querySelector('.gas-settings-content').style.display=isSettings?'block':'none';
            
            // Refresh API key status when opening settings
            if (isSettings) {
        ...
        updateApiKeyStatus() {
            ['gemini', 'cohere', 'huggingface'].forEach(provider => {
                const key = this.settings.getApiKey(provider);
                const statusElement = this.shadow.querySelector(`#${provider}-status`);
                
                if (key && this.settings.validateApiKey(provider, key)) {
                    statusElement.textContent = '✓ Saved and valid';
                    statusElement.className = 'api-key-status valid';
                } else if (key) {
                    statusElement.textContent = '⚠ Saved but invalid format';
                    statusElement.className = 'api-key-status invalid';
                } else {
                    statusElement.textContent = '';
                    statusElement.className = 'api-key-status';
        ...
        populateSettingsUI() {
            ...
            // Load API keys with enhanced retrieval
            this.shadow.querySelector('#gemini-key').value=this.settings.getApiKey('gemini');
            this.shadow.querySelector('#cohere-key').value=this.settings.getApiKey('cohere');
            this.shadow.querySelector('#huggingface-key').value=this.settings.getApiKey('huggingface');
        ...
        handleProviderChange(){
            const p=this.shadow.querySelector('#provider-selector').value;
            this.shadow.querySelector('#gemini-settings').style.display=p==='gemini'?'block':'none';
            this.shadow.querySelector('#cohere-settings').style.display=p==='cohere'?'block':'none';
            this.shadow.querySelector('#huggingface-settings').style.display=p==='huggingface'?'block':'none';
            this.populateModelSelector();
        }
        
        populateModelSelector(){
            const p=this.shadow.querySelector('#provider-selector').value;
            const m={'gemini':['gemini-1.5-flash-latest','gemini-1.5-pro-latest'],'cohere':['command-r-plus','command-r'],'huggingface':['microsoft/DialoGPT-large']};
            this.shadow.querySelector('#model-selector').innerHTML=m[p].map(i=>`<option value="${i}">${i}</option>`).join('');
            const c=this.settings.get('model');
            if(m[p].includes(c)){this.shadow.querySelector('#model-selector').value=c;}else{this.shadow.querySelector('#model-selector').selectedIndex=0;}
        }
        
        saveSettingsFromUI() {
            this.settings.set('provider',this.shadow.querySelector('#provider-selector').value);
            this.settings.set('apiKey',this.shadow.querySelector('#gemini-key').value.trim());
            this.settings.set('cohereKey',this.shadow.querySelector('#cohere-key').value.trim());
            this.settings.set('huggingfaceKey',this.shadow.querySelector('#huggingface-key').value.trim());
            ...
            const btn=this.shadow.querySelector('.gas-save-settings-btn');
            btn.textContent='Saved & Persisted!';
            btn.style.background='#4CAF50';
            this.memory.addTimer(setTimeout(()=>{
                btn.textContent='Save Settings';
                btn.style.background='#007bff';
            },2000));
            
            // Update API key status after save
            this.updateApiKeyStatus();
        }
        
        async sendMessage(isMulti=false){
            const t=this.chatInput.value.trim();
            if(!t&&this.attachedFiles.length===0)return;
            const p=[];
            if(t)p.push({text:t});
            this.attachedFiles.forEach(f=>p.push({inlineData:{mimeType:f.mimeType,data:f.data}}));
            this.addMessageToUI('user',p);
            this.chatHistory.add('user',p);
            this.chatInput.value='';
            this.attachedFiles=[];
            this.updateFilePreview();
            const s=this.shadow.querySelector('.gas-send-btn'),m=this.shadow.querySelector('#multi-send-btn');
            s.textContent='...';s.disabled=true;m.disabled=true;
            if(isMulti){
                await this.executeMultiSend(p);
                s.textContent='Send';s.disabled=false;m.disabled=false;
            }else{
                try{
                    const r=this.settings.get('provider');
                    const o=this.settings.get('model');
                    const i=this.settings.getApiKey(r); // Use enhanced API key retrieval
                    if(!i || i.trim() === '')throw new Error(`${r} API key not set or invalid.`);
                    const a=this.chatHistory.get().map(e=>({role:e.role,parts:e.parts.filter(p=>p.text||p.inlineData)}));
                    const n=await this.api.sendMessage(r,o,a,i);
                    this.addMessageToUI('model',n);
                    this.chatHistory.add('model',n);
                }catch(e){
                    this.addMessageToUI('system',[{text:`Error: ${e.message}`}]);
                }finally{
                    s.textContent='Send';s.disabled=false;m.disabled=false;
                }
            }
        ...
        async executeMultiSend(userParts){
            ...
            const k1=this.settings.getApiKey(p1),k2=this.settings.getApiKey(p2); // Use enhanced API key retrieval
            const m1='gemini-1.5-flash-latest',m2='command-r';
            const h=this.chatHistory.get().map(msg=>({role:msg.role,parts:msg.parts.filter(p=>p.text||p.inlineData)}));
            
            if(!k1||!k2||k1.trim()===''||k2.trim()===''){
                this.addMessageToUI('system',[{text:'API keys for both primary and secondary AI must be set and valid.'}]);
                return;
            }
            
            const results=await Promise.allSettled([
                this.api.sendMessage(p1,m1,h,k1),
                this.api.sendMessage(p2,m2,h,k2)
            ]);
        ...
        addMessageToUI(role,parts,save=true){
            const el=document.createElement('div');
            el.className=`${role}-message`;
            let c='';
            parts.forEach(p=>{
                if(p.text)c+=p.text.replace(/</g,"&lt;").replace(/>/g,"&gt;")+' ';
                if(p.inlineData)c+='[Attachment] ';
            });
            el.innerHTML=c;
            this.chatMessagesDiv.appendChild(el);
            this.chatMessagesDiv.scrollTop=this.chatMessagesDiv.scrollHeight;
        }
        
        addMultiMessageToUI(p1,parts1,p2,parts2){
            const c=document.createElement('div');
            c.className='multi-response-container';
            const t1=parts1[0]?.text||'[No Response]',t2=parts2[0]?.text||'[No Response]';
            c.innerHTML=`<div class="multi-response-column"><div class="multi-response-header">${p1}</div><div>${t1}</div></div><div class="multi-response-column"><div class="multi-response-header">${p2}</div><div>${t2}</div></div>`;
            this.chatMessagesDiv.appendChild(c);
            this.chatMessagesDiv.scrollTop=this.chatMessagesDiv.scrollHeight;
        }
        ...
        async handlePaste(){
            try{
                ...
                if(t)this.chatInput.value+=t;
            }catch(e){
                this.addMessageToUI('system',[{text:'Clipboard read failed.'}]);
            }
        }
        ...
        processFiles(files){
            for(const f of files){
                ...
                r.onload=e=>{
                    this.attachedFiles.push({name:f.name,mimeType:f.type,data:e.target.result.split(',')[1]});
                    this.updateFilePreview();
                };
                r.readAsDataURL(f);
            }
        ...
        updateFilePreview(){
            this.filePreviewArea.innerHTML=this.attachedFiles.map((f,i)=>`<div class="gas-file-preview-item">${f.name}<button data-index="${i}">×</button></div>`).join('');
            this.filePreviewArea.querySelectorAll('button').forEach(b=>this.memory.add(b,'click',e=>{
        ...
        cleanup(isReload=false){
            ...
            this.settings.save();
            this.memory.cleanup(isReload);
            if(this.shadowHost)this.shadowHost.remove();
            if(this.minimizedIcon)this.minimizedIcon.remove();
            window[SECURITY_NAMESPACE]=null;
        }
    }
    
    // Initialize the application with enhanced persistence
    window[SECURITY_NAMESPACE]=new SuiteApplication();
    
    // Add periodic auto-save for API keys
    setInterval(() => {
        if (window[SECURITY_NAMESPACE] && window[SECURITY_NAMESPACE].settings) {
            window[SECURITY_NAMESPACE].settings.save();
        }
    }, 60000); // Auto-save every minute
    
    console.log('theSuite initialized with persistent API key storage');
})();
